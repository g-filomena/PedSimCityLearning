...
public class TimePars {
	...
	/**
	 * Calculates the number of steps in a given time range based on the step
	 * interval. Assumes a step occurs every 20 minutes.
	 */
	...
	public static boolean isOffPeakHours(LocalDateTime currentTime) {
		...
		return (currentTimeOnly.isAfter(nightEnd) && currentTimeOnly.isBefore(morningPeakStart))
				|| (currentTimeOnly.isAfter(morningPeakEnd) && currentTimeOnly.isBefore(TimePars.eveningPeakStart))
