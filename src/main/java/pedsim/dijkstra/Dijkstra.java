...
import pedsim.landmarkNavigation.Landmarkness;
...
public class Dijkstra {
  ...
  protected void initialise(NodeGraph originNode, NodeGraph destinationNode,
    ...
    this.agentNetwork = CommunityCognitiveMap.getCommunityNetwork();
    this.agent = agent;
  ...
  protected void initialiseDual(Set<NodeGraph> centroidsToAvoid, NodeGraph previousJunction) {
    ...
    this.agentDualNetwork = CommunityCognitiveMap.getCommunityDualNetwork();
  ...
  protected DirectedEdge retrieveFromPrimalParentGraph(NodeGraph step) {
    ...
    return CommunityCognitiveMap.getCommunityNetwork().getDirectedEdgeBetween(nodeFrom, nodeTo);
  }
}
