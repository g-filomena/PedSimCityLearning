...
public class LandmarkIntegration {
  ...
  public static void setLocalLandmarksAtJunctions() {
    ...
    List<NodeGraph> nodes = CommunityCognitiveMap.getCommunityNetwork().getNodes();
    nodes.forEach((node) -> {
      List<MasonGeometry> containedLandmarks =
          CommunityCognitiveMap.getLocalLandmarks().featuresWithinDistance(
  ...
  public static void assignAnchoringLandmarksToNodes() {
    ...
    nodes.forEach((node) -> {
      ...
      List<MasonGeometry> potentialAnchors =
          CommunityCognitiveMap.getGlobalLandmarks().featuresWithinDistance(
