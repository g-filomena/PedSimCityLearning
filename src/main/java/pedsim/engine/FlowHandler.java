 ...
 * @param <E>
 */
public class FlowHandler<E extends Enum<E>> {
  ...
  E[] enumAgentScenarios;
  E[] enumScenarios;
  String[] scenarios;
  private PedSimCity state;
  Exporter exporter;

  public FlowHandler(int job, PedSimCity state, E[] agentScenarioValues,
      E[] simulationScenarioValues) {

    this.job = job;
    this.state = state;
    this.enumAgentScenarios = agentScenarioValues;
    this.enumScenarios = simulationScenarioValues;
    scenarios = (enumAgentScenarios != null && enumScenarios != null)
        ? Arrays.stream(enumAgentScenarios)
            .flatMap(agentScenario -> Arrays.stream(enumScenarios)
                .map(simScenario -> agentScenario + "_" + simScenario))
            .toArray(String[]::new)
        : (enumAgentScenarios != null
            ? Arrays.stream(enumAgentScenarios).map(Enum::toString).toArray(String[]::new)
            : Arrays.stream(enumScenarios).map(Enum::toString).toArray(String[]::new));
   ...
   * @throws Exception
   */
  public synchronized void updateCognitiveMapsData(int day, Enum<?> scenarioValue)
      throws Exception {
    ...
    for (Agent agent : state.agentsList) {
      ...
      List<MasonGeometry> buildingGeoemetries =
          CommunityCognitiveMap.getBuildings().getGeometriesFromIDs(buildings);
