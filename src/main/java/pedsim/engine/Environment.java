...
import pedsim.communityCognitiveMap.BarrierIntegration;
...
public class Environment {
  ...
  static private void prepareBuildings() {
    ...
    for (final MasonGeometry buildingGeometry : geometries) {
      ...
      building.attributes.put("DMA", buildingGeometry.getAttributes().get("DMA"));
      building.attributes.put("globalLandmarkness",
          buildingGeometry.getAttributes().get("gScore_sc"));
      building.attributes.put("localLandmarkness",
          buildingGeometry.getAttributes().get("lScore_sc"));
    ...
    PedSimCity.network.getNodes().forEach((node) -> {
      ...
      for (MasonGeometry building : nearestBuildings) {
        ...
        final String DMA =
            PedSimCity.buildingsMap.get(buildingID).attributes.get("DMA").getString();
        node.DMA = DMA;
      }
    });
  }
